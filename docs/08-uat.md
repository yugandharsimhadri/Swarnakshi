...
The run uses a real PostgreSQL database, because the product runs on PostgreSQL and an acceptance
suite that proves it works on a different engine has proved the wrong thing. EF creates the database
on the first migrate; `ApiServer.DisposeAsync` drops it `WITH (FORCE)` so a lingering pooled
connection cannot block the DROP.
