...
const COMPANY_CODE = process.env.SWK_CODE ?? "scops";
const COMPANY_NAME = "SCOPS Constructions";
...
async function build() {
  ...
  // ---- villas -------------------------------------------------------------
  // Three handed over, three at 60%, three just started, one still on paper.
  ...
  const villaSpec = [
    ...
    [greenMeadows, "Villa 104", "104", customers.naidu, 4_400_000, 5_800_000, 1, 60, 190],
    [greenMeadows, "Villa 105", "105", customers.sharma, 4_400_000, 5_800_000, 1, 60, 185],
    [greenMeadows, "Villa 106", "106", null, 4_600_000, null, 1, 60, 180],
  ...
  const done = at(100), half = at(60), early = at(10);
  ...
  // Third and fourth lots at moving rates - the weighted average now has four inputs per material
  // on Green Meadows, which is what makes the issue rates below worth checking.
  await buy(greenMeadows, sriBalaji, 95, [[cement, 640, 462], [steel, 3800, 69]], "SBT/24-25/588", "Rate up again after the budget.");
  await buy(greenMeadows, lakshmi, 80, [[sand, 480, 56], [aggregate, 560, 47], [brick, 18000, 10]], "LBM/1502", "Short lorry, gate slip 4417.");
  // 104-106 reach plumbing and conduit at 60%, and the first lot was sized for 101-103 only.
  await buy(greenMeadows, anjaneya, 88, [[wire, 2600, 36], [pipe, 700, 99]], "AH/2468", "Second wiring lot for 104-106.");
  await buy(palmGrove, lakshmi, 75, [[sand, 420, 55], [aggregate, 500, 46], [brick, 15000, 10.5]], "LBM/1516", null);
  await buy(palmGrove, anjaneya, 45, [[tiles, 900, 84], [paint, 130, 298]], "AH/2455", "Finishing lot held in store for 201.");
  ...
  for (const [i, v] of half.entries()) {
    ...
    // 60% means the roof is on and the first finishing trades are in - so plumbing and
    // wiring are consumed, but no tiles or paint yet.
    await issue(v, 105 - i * 5, [[cement, 90], [brick, 3200], [pipe, 120], [wire, 420]], "Second slab, plumbing and conduit.");
  }
  ...
  // ---- spend that belongs to the site, not to any one villa ---------------
  // A watchman guards the whole compound and a borewell serves every villa on it. Charging
  // either to one villa would overstate that villa and understate the rest, so these sit at
  // site level and appear in the site report as overhead, outside any villa's cost.
  console.log("Recording site-level overheads…");
  const siteExpense = async (site, h, amount, day, note) =>
    post("/site-expenses", {
      siteId: site.id, date: daysAgo(day), expenseHeadId: h.id,
      description: note, amount, paymentStatus: 2, paymentMethodId: cash.id,
    });
  await siteExpense(greenMeadows, hMisc, 96_000, 200, "Watchman, 8 months.");
  await siteExpense(greenMeadows, hMisc, 145_000, 260, "Borewell and compound wall.");
  await siteExpense(greenMeadows, hTransport, 38_000, 150, "Site office cabin shifted.");
  await siteExpense(palmGrove, hMisc, 72_000, 140, "Watchman, 6 months.");
  await siteExpense(palmGrove, hMisc, 118_000, 195, "Borewell.");
...
async function report() {
  ...
  // Read straight from the reports the product itself serves, not recomputed here: the point is
  // to prove the screens agree with each other, so anything this recomputes proves nothing.
  const col = (t, name) => t.columns.findIndex((c) => c.toLowerCase() === name.toLowerCase());
  const sum = (t, name) => {
    const i = col(t, name);
    if (i < 0) throw new Error(`no "${name}" column in [${t.columns.join(", ")}]`);
    return t.rows.reduce((a, r) => a + (typeof r[i] === "number" ? r[i] : 0), 0);
  };

  const register  = await get("/reports/inventory/purchase-register");
  const stockRpt  = await get("/reports/inventory/stock");
  const consumeR  = await get("/reports/inventory/consumption");
  const siteRpt   = await get("/reports/site/summary");
  const costRpt   = await get("/reports/project/cost-summary");
  const companyR  = await get("/reports/company/summary");
  const metric = (name) => {
    const row = companyR.rows.find((r) => String(r[0]).toLowerCase() === name.toLowerCase());
    if (!row) throw new Error(`no "${name}" in the company summary`);
    return Number(row[1]) || 0;
  };

  // Money is decimal(18,2) and weighted-average rates round per movement, so equality is to
  // the rupee, not to the paisa. Anything bigger than a rupee is a real disagreement.
  const checks = [];
  const check = (name, left, right, leftLabel, rightLabel) => {
    const diff = Math.round((left - right) * 100) / 100;
    checks.push({ name, left, right, diff, ok: Math.abs(diff) <= 1, leftLabel, rightLabel });
  };

  //  Every rupee of material bought is either still in a store or has been charged to a villa.
  //  The purchase register's *Total* is the landed value - Sub Total is before tax, and comparing
  //  that one is what made this look broken before.
  check("Material bought = in stock + consumed",
        sum(register, "Total"), sum(stockRpt, "Value") + sum(consumeR, "Value"),
        "purchase register (landed)", "stock on hand + consumed into villas");

  //  The two places the product states total purchases must agree.
  check("Purchase register = company summary",
        sum(register, "Total"), metric("Total purchase (posted)"),
        "purchase register", "company summary");

  //  Stock on hand, per the ledger report and per the roll-up.
  check("Stock value = company summary",
        sum(stockRpt, "Value"), metric("Inventory value (in stock)"),
        "stock report", "company summary");

  //  Consumption is the only thing that turns bought material into project cost.
  check("Consumption = material in project cost",
        sum(consumeR, "Value"), metric("Material consumed (project cost)"),
        "consumption register", "company summary");

  //  Total project cost is the sum of its four buckets and nothing else.
  check("Project cost = material + labour + contractor + other",
        metric("Total project cost"),
        metric("Material consumed (project cost)") + metric("Labour cost") +
          metric("Contractor cost") + metric("Other project expenses"),
        "company summary total", "its four buckets");

  //  Per-villa actuals must roll up to the same number.
  check("Sum of villa costs = total project cost",
        sum(costRpt, "Actual"), metric("Total project cost"),
        "project cost summary", "company summary");

  //  Site cost is its villas plus what belongs to the site itself, not to any villa.
  check("Site villa cost + overhead = total cost",
        sum(siteRpt, "Villa Cost") + sum(siteRpt, "Site Overhead"),
        metric("Total cost incl. site overhead"),
        "site summary", "company summary");

  //  Capital employed is what the site is holding: built cost plus unissued stock.
  check("Capital employed = villa cost + overhead + stock",
        sum(siteRpt, "Capital Employed"),
        sum(siteRpt, "Villa Cost") + sum(siteRpt, "Site Overhead") + sum(siteRpt, "Stock Value"),
        "site summary column", "its own components");

  //  Stock is held at a site, so the two views of it must agree.
  check("Stock by site = stock by material",
        sum(siteRpt, "Stock Value"), sum(stockRpt, "Value"),
        "site summary", "stock report");

  //  What customers still owe.
  check("Sale - received = outstanding",
        metric("Customer sale value") - metric("Customer receipts"),
        metric("Customer outstanding"),
        "sale less receipts", "company summary");
  ...
  for (const c of checks) {
    console.log(
      "  " + (c.ok ? "OK  " : "FAIL") + "  " + pad(c.name, 46) +
      rpad(money(c.left), 15) + rpad(money(c.right), 15) +
      (c.ok ? "" : `  off by ${money(c.diff)}`),
    );
  }
  const failed = checks.filter((c) => !c.ok);
  console.log();
  if (failed.length === 0) {
    console.log(`  All ${checks.length} identities hold.`);
  } else {
    console.log(`  ${failed.length} of ${checks.length} FAILED:`);
    for (const c of failed) {
      console.log(`    ${c.name}`);
      console.log(`      ${c.leftLabel}: ${money(c.left)}`);
      console.log(`      ${c.rightLabel}: ${money(c.right)}`);
    }
    process.exitCode = 1;
  }
  console.log();
